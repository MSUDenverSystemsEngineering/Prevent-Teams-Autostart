...
Try {
	...
	[string]$appName = 'Prevent Teams Autostart'
	[string]$appVersion = ''
	[string]$appArch = ''
	[string]$appLang = 'EN'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Show-InstallationWelcome -CloseApps 'teams' -CheckDiskSpace -PersistPrompt
		...
		## <Perform Installation tasks here>
			...
			#Make MSUDenver folder if it does not exist
		if (!(Test-Path -path 'C:\MSUDenver\')) {New-Item 'C:\MSUDenver\' -Type Directory}
			#Copy ps1 to run for each user
		Copy-File -Path "$dirFiles\preventteams.ps1" -Destination "C:\MSUDenver\preventteams.ps1"

			#Registry Settings to replicate across users

		[scriptblock]$HKCURegistrySettings = {
			Set-RegistryKey -Key 'HKCU\SOFTWARE\Policies\Microsoft\Office\16.0\Teams' -Name 'PreventFirstLaunchAfterInstall' -Type 'DWord' -Value '1' -SID $UserProfile.SID
			Set-RegistryKey -Key 'HKCU\Software\Microsoft\Windows\CurrentVersion\RunOnce' -Name 'Prevent Teams Autostart' -Value 'C:\WINDOWS\system32\WindowsPowerShell\v1.0\powershell.exe -noprofile -sta -WindowStyle Hidden -File `"C:\MSUDenver\preventteams.ps1`"' -SID $UserProfile.SID

			}

			# Set script to run on all existing users and the default user for new ones
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCB/5kXVq1v22X5i
# iO3Oj+qYOP3sXXJMNQhcnAaGzK3BAKCCH9IwggVvMIIEV6ADAgECAhBI/JO0YFWU
...
# DQEJBDEiBCC/0uC+g2T9ywgPIzXRLobevDMqQHSNZntbzzMKACEFWDANBgkqhkiG
# 9w0BAQEFAASCAYBY6mRawXnUyE189dyWnQlNxI5Ep/r6X4LEBStTTSDcGWbMPczY
# xqm1C8bf3ADOCq0Q+26Btvnb103B2Z0vpw+hxmSDIz8zB5bY7hpB3pLEKbWHAwaL
# Fjx9zPuUGZ7hqG5klYsCDNz8gTV9P5PRTCG+cidVMb6puepspYjUvNhZsO98iMEl
# ftaXluSHOcjjxtLQ2F3CA6LFisIPM2UjCe7pFsZx5fmxmMD+DKf7MAiM3i2v5rNv
# bHsYkyWHX3a89O27N3q1BdFllY/8xOu6tI0Ld02wdbNBqWvQrrytx8xGIo+f6YKN
# 3zby3FZa3uI5WA5ObE/O2VSwY+ZY6gWe13nGISJi8Ld5osjvkyAirsswSQDJuHu4
# 9g2lDWO2FCXJcTLHJU3A2VkcDHYRuuIaYlLvihB6lpbzKjPabef+Q9B9jVYbEbE+
# Lv2smMauF4qc/QgB3sfaKHG3Y5H3LepZCwILqx6njLrvhokWJ/ImDpAO21sOpGtI
# j+ida4w8X7M+6AChggNMMIIDSAYJKoZIhvcNAQkGMYIDOTCCAzUCAQEwgZIwfTEL
...
# hvcNAQkFMQ8XDTIyMDQyNjIwMjUyOVowPwYJKoZIhvcNAQkEMTIEMOSvZeF1b2D/
# mvDXe2dofyGtnoN9XqM+yslOZST0+cWEODUwL13VeWnW6o42OuPgfTANBgkqhkiG
# 9w0BAQEFAASCAgBUKFIwrbyZoI+0OYh601zNskyapZWOT1TcluZr+W4HGAa8TxHb
# +iRdYmlhQAWjFKd98TGzWE7ndyqd0Q+8PfpLXS4hqoDK6iy8rFwkWSPwd5Vow6JU
# zQk1U7TD9RM3BkN15LhbBK0hoLKZxw6FOW+TDs5TA4EkWx9yQ3dKlUOBoXOwYBl6
# KKrBfpX8JT3UgEBF8ysU/5B6F9f15Bt8yOxomeQHsI44oHAjqezCrCsKHrMH7KJ9
# 0mxocZjxFwR39kc6rvrGFScmo5rYBf/2W7OiAsv4gI3t7nSvHn33RiifH9OHq6nq
# CnjVCz8G856mR/BFxqGhRHn9+l+4POkagFUc2rtAeeSsCIcSXkgyACn6/qoiOptE
# 0Rm5HJu+d1/99wx045Q1kIhHG2n/CVKri2DxYOwkH2kjvA/7kKlnCA87DKqSEpiM
# 1g+ukXYKHy9Ep1336HGNJm/rRHEkPokGtzNwrlR+9aBz02dC4Y67oXFm8StkrGJA
# q5lnz3XIW+FhcHc4IloUeHVfVYESuMoaof2u10YZDlzfq+4dwH47YIZdQSPl+VXh
# xsaYWnINcyvFNm9bqa/zR1J3+6Zh5YHkyA4+qEpOl3z7tYDVgAptGaubDOArhvSz
# a2wxZ7yqgoE6Sm+A8xcUlCdyB99XK8ra29ZKDYslctBcAVtpVpzV52J/aA==
